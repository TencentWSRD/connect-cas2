 ...
 * @param options
 */
function validate(req, callback, options) {
  // check ticket first`
    ...
    validateTicket(req, options, function(err, response) {
      ...
      if (response.status === 200) {
        ...
        parseCasResponse(response.body, logger, function(err, info) {
          ...
          if (!pgtIou) {
            if (options.paths.proxyCallback) {
              ...
              req.session.save(function(err) {
                ...
                lastUrl = getLastUrl(req, options, logger, lastUrl);
          ...
          retrievePGTFromPGTIOU(req, callback, pgtIou, options);
        });
      }
      else {
 ...
 * @param options
 */
function retrievePGTFromPGTIOU(req, callback, pgtIou, options) {
  var logger = utils.getLogger(req, options);
...
function getLastUrl(req, options, logger, lastUrl) {
  if (typeof options.redirect === 'function') {
    var customRedirectUrl;
    if (customRedirectUrl = options.redirect(req)) {
