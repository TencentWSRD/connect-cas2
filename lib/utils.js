...
function getPath(name, options) {
  ...
  switch (name) {
    ...
    case 'pgtUrl':
      ...
      path = (proxyCallbackUri.protocol && proxyCallbackUri.host) ? (options.paths.proxyCallback) : ((options.proxyCallbackPrefix || options.servicePrefix) + options.paths.proxyCallback);
      break;
